...
public final class HttpRequestDecoderFilter extends SocketFilter {

	private static final Logger log = LogFactory.getLogger();

	@Override
	public void onSocketChannelOpen(SelectionKey selectionKey) {
		selectionKey.attach(SelectionKeyAttachment.getHttpAttachment());
	...
	private final HttpRequest decodeRequestHeader(SelectionKey selectionKey,String headerData){
		...
		try{
			...
			return HttpRequest.getHttpRequest().setMap(map);
	...
	public void close(SelectionKey selectionKey) {
		SelectionKeyAttachment attachment = (SelectionKeyAttachment) selectionKey.attachment();
		if (attachment!=null) {
			attachment.getHttpRequest().recycle();
		}
	...
	private final void decodeGetRequest(SocketFilter nextFilter,SelectionKey selectionKey,ByteBuffer dataBuf){
		
		HttpRequest request;
		try {
			request = decodeRequestHeader(selectionKey, PWSConfig.CHARSET_DECODER.decode(dataBuf).toString());
			if (request!=null) {
	...
	private boolean requestComplete(SelectionKey selectionKey,ByteBuffer dataBuf){
		...
		if (dataBuf.get(0) == (byte) 'G' && dataBuf.get(1) == (byte) 'E'
			...
			for (int i = last; i > 3; i--) {
				if (dataBuf.get(i) == (byte) 0x0A && dataBuf.get(i - 1) == (byte) 0x0D
						&& dataBuf.get(i - 2) == (byte) 0x0A
						&& dataBuf.get(i - 3) == (byte) 0x0D) {
					postHeaderEnd = i + 1;
					break;
				}
			}
